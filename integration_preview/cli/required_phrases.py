...
def is_rule_candidate_for_ml_suggestion(rule, licenses_by_key):
    """
    Return True if ``rule`` is suitable for ML-based required phrase suggestion.
    """
    if rule.is_from_license:
        return False
    ...
    if rule.is_false_positive:
        return False
    if rule.is_generic(licenses_by_key):
        return False
...
def collect_candidate_phrases_for_rule(rule, licenses_by_key):
    """
    Return candidate phrase strings for a ``rule`` using conservative heuristics.

    This is a prototype placeholder for ML predictions. It intentionally reuses
    stable phrase sources as pseudo-predictions with explicit confidence scores.
    """
    candidates = []
    expr = rule.license_expression

    # Source 1: license attributes
    lic = licenses_by_key.get(expr)
    if lic:
        for val in [lic.name, lic.short_name, lic.spdx_license_key] + (lic.other_spdx_license_keys or []):
            if not val:
                continue
            val = val.strip()
            if not val:
                continue
            if 'licenseref-scancode' in val.lower():
                continue
            if find_phrase_spans_in_text(rule.text, val):
                candidates.append((val, 0.90, 'license_attributes'))

    # Source 2: any required phrase rules from same expression (transfer baseline)
    # Reuse current collection logic by expression when available
    rules_by_expression = get_rules_by_expression()
    same_expr_rules = rules_by_expression.get(expr, [])
    for sr in same_expr_rules:
        if sr.is_required_phrase and sr.text:
            phrase = sr.text.strip()
            if find_phrase_spans_in_text(rule.text, phrase):
                candidates.append((phrase, 0.85, 'required_phrase_rule'))

    # de-duplicate while keeping best confidence
    by_norm = {}
    for phrase, conf, source in candidates:
        norm = get_normalized_text(phrase)
        prev = by_norm.get(norm)
        if not prev or conf > prev[1]:
            by_norm[norm] = (phrase, conf, source)

    return sorted(by_norm.values(), key=lambda x: x[1], reverse=True)


def collect_candidate_phrases_for_rule_by_model(rule, licenses_by_key, model='deberta-v3'):
    """
    Return candidate phrases for ``rule`` based on selected ``model`` strategy.

    This prototype supports:
    - ``deberta-v3``: conservative attribute + required_phrase transfer blend
    - ``codebert``: similar sources with slightly lower confidence priors
    - ``starcoder``: exploratory mode with review-first conservative priors
    """
    candidates = collect_candidate_phrases_for_rule(rule, licenses_by_key)
    ...
    if model == 'codebert':
        adjusted = []
        for phrase, conf, source in candidates:
            adjusted.append((phrase, max(0.0, conf - 0.03), f'{source}:codebert'))
        return adjusted

    if model == 'starcoder':
        adjusted = []
        for phrase, conf, source in candidates:
            adjusted.append((phrase, max(0.0, conf - 0.08), f'{source}:starcoder'))
        return adjusted

    raise click.UsageError(
        f'Unsupported model: {model!r}. Use one of: deberta-v3, codebert, starcoder'
    )


def collect_candidate_phrases_for_rule_hf(rule, model_path):
    """
    Return phrase candidates from a Hugging Face token-classification model.

    This expects a model fine-tuned for BIO labels where labels include B-REQ/I-REQ.
    """
    try:
        from transformers import pipeline
    except ImportError as e:
        raise click.UsageError(
            'transformers is required for --hf-model-path. Install with: '
    ...
    for ent in entities:
        ...
        # normalize HF subword artifacts
        phrase = phrase.replace('##', '').replace('▁', ' ').replace('Ġ', ' ').strip()
        if not phrase:
            continue
        ...
        conf = float(ent.get('score') or 0.0)
        candidates.append((phrase, conf, 'hf_model'))

    # dedupe with best confidence
    by_norm = {}
    for phrase, conf, source in candidates:
        norm = get_normalized_text(phrase)
        prev = by_norm.get(norm)
...
def bucket_ml_suggestions(candidates, threshold_high=0.95, threshold_low=0.80, max_auto_apply=2):
    """
    Bucket ``candidates`` into auto_apply/review/reject by confidence.
    """
    auto_apply = []
    review = []
    reject = []
    ...
    for phrase, conf, source in candidates:
        if conf >= threshold_high and len(auto_apply) < max_auto_apply:
            auto_apply.append((phrase, conf, source))
        elif conf >= threshold_low:
            review.append((phrase, conf, source))
        else:
            reject.append((phrase, conf, source))

    return auto_apply, review, reject


def apply_phrase_candidates_to_rule(rule, candidates, dry_run=False):
    """
    Apply phrase ``candidates`` to ``rule`` and return count of applied phrases.
    """
    applied = 0
    for phrase, _conf, source in candidates:
        did_update = add_required_phrase_to_rule(
            rule=rule,
            required_phrase=phrase,
            source=f'ml-prototype:{source}',
            debug=False,
            dry_run=dry_run,
        )
        if did_update:
...
def write_review_artifact(review_data, review_out):
    """
    Write JSON review artifact to ``review_out`` path.
    """
    import json
    ...
    out.write_text(json.dumps(review_data, indent=2, ensure_ascii=False))


def get_rules_by_expression_for_dir(selected_rules_dir=None, license_expression=None):
    """
    Return rules grouped by expression from an optional ``selected_rules_dir``.
    """
    if selected_rules_dir:
        grouped = defaultdict(list)
        for rule in load_rules(rules_data_dir=selected_rules_dir):
            if license_expression and rule.license_expression != license_expression:
                continue
            grouped[rule.license_expression].append(rule)
        return grouped

    grouped = get_rules_by_expression()
    if license_expression:
        grouped = {license_expression: grouped.get(license_expression, [])}
    return grouped


@click.command(name='review-required-phrases-ml')
@click.option(
    '--review-in',
    type=click.Path(exists=True, dir_okay=False),
    required=True,
    help='Path to the review artifact JSON generated by suggest-required-phrases-ml.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--approved-out',
    ...
    required=True,
    help='Path to write approved suggestions JSON.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--interactive',
    is_flag=True,
    default=False,
    help='Enable interactive accept/reject prompts for each review suggestion.',
    cls=PluggableCommandLineOption,
)
@click.help_option('-h', '--help')
def review_required_phrases_ml(review_in, approved_out, interactive):
    """
    Prototype minimal review UI for ML required phrase suggestions.

    This is a CLI-based interactive review workflow to approve/reject phrases
    from a generated review artifact.
    """
    import json
    from pathlib import Path

    review_path = Path(review_in)
    data = json.loads(review_path.read_text())
    rules = data.get('rules', [])

    approved = {'meta': data.get('meta', {}), 'rules': []}
    reviewed_count = 0

    for rule in rules:
        rid = rule.get('rule_identifier')
        lex = rule.get('license_expression')
        accepted_for_rule = []

        for rec in rule.get('review', []):
            ...
            if interactive:
                click.echo(f'Rule: {rid} | expression: {lex}')
                click.echo(f'  Suggestion: {phrase!r} (confidence={conf}, source={source})')
                accept = click.confirm('  Accept this suggestion?', default=False)

            if accept:
                accepted_for_rule.append(rec)
            reviewed_count += 1

        if accepted_for_rule:
            approved['rules'].append({
                'rule_identifier': rid,
                'license_expression': lex,
                'approved': accepted_for_rule,
            })

    out = Path(approved_out)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(approved, indent=2, ensure_ascii=False))
    click.echo(f'Reviewed suggestions: {reviewed_count}')
    click.echo(f'Approved rules written to: {approved_out}')


@click.command(name='suggest-required-phrases-ml')
@click.option(
    '--license-expression',
    type=str,
    default=None,
    help='Optional license expression filter.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--rules-dir',
    ...
    default=None,
    help='Optional rules directory for fast prototype runs on a subset.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--max-rules',
    ...
    default=0,
    show_default=True,
    help='Optional maximum number of candidate rules to process (0 means no limit).',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--model',
    type=click.Choice(['deberta-v3', 'codebert', 'starcoder'], case_sensitive=False),
    default='deberta-v3',
    show_default=True,
    help='Prototype model strategy for phrase suggestion.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--hf-model-path',
    type=str,
    default=None,
    help='Optional Hugging Face model path (local dir or model id) for real token-classification inference.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--threshold-high',
    ...
    default=0.95,
    show_default=True,
    help='High confidence threshold for auto-apply.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--threshold-low',
    ...
    default=0.80,
    show_default=True,
    help='Low confidence threshold for review queue.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--max-auto-apply',
    ...
    default=2,
    show_default=True,
    help='Maximum phrases auto-applied per rule.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--dry-run',
    is_flag=True,
    default=False,
    help='Do not write any rule updates to disk.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--review-out',
    type=str,
    default=None,
    help='Optional path to write review queue JSON artifact.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--validate',
    is_flag=True,
    default=False,
    help='Validate rules after processing.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--reindex',
    is_flag=True,
    default=False,
    help='Rebuild/cached license index after processing.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--verbose',
...
@click.help_option('-h', '--help')
def suggest_required_phrases_ml(
    license_expression,
    rules_dir,
    max_rules,
    model,
    ...
    """
    Prototype ML-assisted required phrase suggestion command.

    This command intentionally uses conservative pseudo-ML confidence scoring and
    produces review artifacts to demonstrate the full workflow described in the
    proposal (auto-apply/review/reject buckets).
    """
    if threshold_low > threshold_high:
        raise click.UsageError('--threshold-low cannot be greater than --threshold-high')

    licenses_by_key = get_licenses_db()
    rules_by_expression = get_rules_by_expression_for_dir(
        selected_rules_dir=rules_dir,
        license_expression=license_expression,
    )

    expressions = sorted(rules_by_expression)
    ...
    for expr in expressions:
        for rule in rules_by_expression.get(expr, []):
            if max_rules and processed_rules >= max_rules:
                break
            if not is_rule_candidate_for_ml_suggestion(rule, licenses_by_key):
                continue

            processed_rules += 1
            if hf_model_path:
                candidates = collect_candidate_phrases_for_rule_hf(
                    rule=rule,
                    model_path=hf_model_path,
                )
            else:
                candidates = collect_candidate_phrases_for_rule_by_model(
                    rule=rule,
            ...
            total_candidates += 1
            auto_apply, review, reject = bucket_ml_suggestions(
                candidates,
                threshold_high=threshold_high,
                threshold_low=threshold_low,
                max_auto_apply=max_auto_apply,
            ...
            applied_count = apply_phrase_candidates_to_rule(rule, auto_apply, dry_run=dry_run)
            total_auto_applied += applied_count

            review_data['rules'].append({
                'rule_identifier': rule.identifier,
                'license_expression': rule.license_expression,
                'auto_apply': [
                    {'phrase': p, 'confidence': c, 'source': s}
                    for p, c, s in auto_apply
                ],
                'review': [
                    {'phrase': p, 'confidence': c, 'source': s}
                    for p, c, s in review
    ...
    if review_out:
        write_review_artifact(review_data, review_out)

    if verbose:
        click.echo(f'Prototype ML suggestion rules processed: {total_candidates}')
        click.echo(f'Prototype ML auto-applied phrases: {total_auto_applied}')
        click.echo(f'Prototype candidate rules traversed: {processed_rules}')
        if review_out:
            click.echo(f'Review artifact written to: {review_out}')

    validate_and_reindex(validate=validate, reindex=reindex, verbose=verbose)


@click.command(name='apply-approved-required-phrases-ml')
@click.option(
    '--approved-in',
    type=click.Path(exists=True, dir_okay=False),
    required=True,
    help='Path to approved suggestions JSON from review-required-phrases-ml.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--rules-dir',
    type=click.Path(exists=True, file_okay=False),
    default=None,
    help='Optional rules directory to apply changes on a subset.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--dry-run',
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help='Validate rules after applying approved suggestions.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--reindex',
    is_flag=True,
    default=False,
    help='Rebuild/cached license index after applying approved suggestions.',
    cls=PluggableCommandLineOption,
)
@click.option(
    '--verbose',
...
@click.help_option('-h', '--help')
def apply_approved_required_phrases_ml(approved_in, rules_dir, dry_run, validate, reindex, verbose):
    """
    Apply approved ML phrase suggestions produced by review-required-phrases-ml.
    """
    import json
    from pathlib import Path

    data = json.loads(Path(approved_in).read_text())
    grouped = get_rules_by_expression_for_dir(selected_rules_dir=rules_dir)
    rules_by_identifier = {}
    for _expr, rules in grouped.items():
        for rule in rules:
            rules_by_identifier[rule.identifier] = rule

    applied_rules = 0
    applied_phrases = 0

    for item in data.get('rules', []):
        rid = item.get('rule_identifier')
        rule = rules_by_identifier.get(rid)
        if not rule:
            continue

        applied_for_rule = 0
        for approved in item.get('approved', []):
            phrase = (approved.get('phrase') or '').strip()
            if not phrase:
                continue
            did_update = add_required_phrase_to_rule(
                rule=rule,
                required_phrase=phrase,
                source='ml-prototype:approved',
                debug=False,
                dry_run=dry_run,
            )
            if did_update:
    ...
    if verbose:
        click.echo(f'Applied phrases: {applied_phrases}')
        click.echo(f'Rules updated: {applied_rules}')
